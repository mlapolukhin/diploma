...
def download_one_image(bucket, split, image_id, download_folder):
    if os.path.exists(os.path.join(download_folder, f"{image_id}.jpg")):
        return
    try:
...
def download_images(
    image_list: List[Tuple[str, str]], num_processes: int, download_folder: str
):
    """
    Downloads all images specified in the input file.
    image_list is [split, ImageID] view
    """
    bucket = boto3.resource(
        "s3", config=botocore.config.Config(signature_version=botocore.UNSIGNED)
    ).Bucket(BUCKET_NAME)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_list",
        type=str,
        default="../../data/tmp/openimages/images.txt",
        help=(
    ...
    parser.add_argument(
        "--download_folder",
        type=str,
        default="../../data/tmp/openimages/downloads/images/",
